...
func (r *NeutronAPIReconciler) reconcileNormal(ctx context.Context, instance *neutronv1beta1.NeutronAPI, helper *helper.Helper) (ctrl.Result, error) {
	...
	transportURL, _, err := r.transportURLCreateOrUpdate(
		ctx, instance, instance.Spec.RabbitMqClusterName, instance.Spec.RabbitMqClusterName)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
	...
	if notificationBusName != "" {
		notificationTransportURL, _, err := r.transportURLCreateOrUpdate(
			ctx, instance, notificationBusName+"-notifications", notificationBusName)
		...
		if !notificationTransportURL.IsReady() {
			Log.Info(fmt.Sprintf("Waiting for Notificaitons TransportURL %s secret to be created", notificationTransportURL.Name))
		...
		Log.Info(fmt.Sprintf("Notificaitons TransportURL %s secret created Successfully ", notificationTransportURL.Name))
		...
		for _, url := range transportURLList.Items {
			if strings.Contains(url.Name, "neutron") && strings.Contains(url.Name, "notifications") {
				transporURLName = url.Name
			}
		}
		if transporURLName != "" {
			err = r.transportURLDeleted(ctx, instance, transporURLName)
			if err != nil {
				return ctrl.Result{}, err
			}
	...
	*rabbitmqv1.TransportURL, controllerutil.OperationResult, error) {
	...
	transportURL := &rabbitmqv1.TransportURL{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("neutron-%s-transport", transporturlName),
			Namespace: instance.Namespace,
		},
	}
...
func (r *NeutronAPIReconciler) transportURLDeleted(
	...
	if err != nil {
		Log.Info(fmt.Sprintf("XXX Could not delete TransportURL %s err: %s", transportURLName, err))
		return err
	}

	Log.Info("XXX Deleted transportURL", ":", transportURLName)
...
func (r *NeutronAPIReconciler) generateServiceSecrets(
	...
	if instance.Status.NotificationsTransportURLSecret != nil {
		notificationsTransportURL, _, err := secret.GetDataFromSecret(
			ctx, h, *instance.Status.NotificationsTransportURLSecret, time.Second, "transport_url")
