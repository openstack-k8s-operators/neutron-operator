...
func (r *NeutronAPIReconciler) transportURLDeleted(
	...
	if err != nil {
		if k8s_errors.IsNotFound(err) {
			Log.Info(fmt.Sprintf("TransportURL %s not found to delete, considering deleted already", transportURLName))
			return nil
		}
...
func (r *NeutronAPIReconciler) generateServiceSecrets(
	...
	transportURL, err := r.getTransportURL(ctx, h, instance, &instance.Status.TransportURLSecret)
	if err != nil {
		// If transport URL is not ready yet, we can't generate the main config
		// but we should not return an error - instead wait for it on reconcile
		return nil
	}
